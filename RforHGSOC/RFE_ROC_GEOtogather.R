...
for (i in 1:n) {
  ...
  ## train
  set.seed(666) 
  # tuned <- tune.svm(x, y1, gamma = 10^(-6:-1), cost = 10^(1:2)) # tune
  ...
  # glmfit <- glm(y~., data = x, family = binomial)
  # # glmfit <- glm(y~., data = x, family = binomial, control = list(maxit = 100))
  # summary(glmfit)
  # p_test <- predict(glmfit, xtest, type = "response")
