...
# SVMrfe <- as.matrix(read.csv(file = dir[7],header=TRUE, sep = ','))
# XGBrfe <- as.matrix(read.csv(file = dir[8],header=TRUE, sep = ','))

allmethod <- cbind(Biorfe, ABrfe, GBMrfe, NNETrfe, SVMrfe, XGBrfe)
colnames(allmethod) <- c("Bio-RFE", "AB-RFE", "GBDT-RFE", "NNET-RFE", "SVM-RFE", "XGB-RFE")
# write.csv(allmethod, file = "datanew\\AllMethodUpset.csv", row.names = F)



features <- list(Biorfe, ABrfe, GBMrfe, NNETrfe, SVMrfe, XGBrfe)
listname <- alist(Biorfe, ABrfe, GBMrfe, NNETrfe, SVMrfe, XGBrfe)
...
# Stability <- c(mycomb(n,k2,A2), mycomb(n,k3,A3), mycomb(n,k4,A4), mycomb(n,k5,A5), mycomb(n,k6,A6))
ll <- length(Stability)
...
A <- list(A2,A3,A4,A5,A6,A7)
l <- which(Stability == max(Stability))
l
A[[6]][,7]
order(Stability, decreasing = T)
sort(Stability, decreasing = T)
View(cbind(order(Stability, decreasing = T),sort(Stability, decreasing = T)))
Stability[l]
